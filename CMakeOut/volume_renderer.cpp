...
namespace pipelines
{
    ...
    const volume_renderer::pipeline_data& volume_renderer::get_or_create_pipeline(const output_specs& output_specs) {
        if (!_pipelines.contains(output_specs)) {
            ...
            //Create VertexBuffer for a quad 

            VkBufferCreateInfo bufferInfo = util::vk::initializers::bufferCreateInfo(buffer_usage, data_size);
            bufferInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
            bufferInfo.size = sizeof(float) * 4 * 2; // 4 2D vertices
            bufferInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
            bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;

            util::vk::create_buffer(&bufferInfo);
